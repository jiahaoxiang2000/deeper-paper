...
%DIF DEL /tmp/index-old.tex   Mon Dec 29 10:39:55 2025
%DIF ADD /tmp/index-new.tex   Mon Dec 29 10:39:55 2025
...
%%%% 6. ABSTRACT %%%%
\begin{abstract}
	Large-scale quantum computers necessitate migration of Internet of Things (IoT) systems to post-quantum cryptographic standards. While NIST has standardized ML-DSA (Module-Lattice-Based Digital Signature Algorithm) for digital signatures, practical deployment of post-quantum authentication in resource-constrained IoT environments remains inadequately characterized. \DIFaddbegin \DIFadd{This work evaluates }\DIFaddend ML-DSA integration within MQTT-based IoT systems \DIFdelbegin \DIFdel{is evaluated }\DIFdelend through comprehensive performance analysis on ARM Cortex-M4 microcontrollers \DIFaddbegin \DIFadd{and proposes an adaptive security level selection protocol that dynamically selects ML-DSA parameter sets based on message criticality and device resource state}\DIFaddend . The methodology encompasses signature generation and verification benchmarking, memory utilization analysis, and protocol overhead assessment under realistic IoT constraints. Performance \DIFdelbegin \DIFdel{implications of ML-DSA deployment are examined through comparison with classical signature schemes,quantifying computational overhead , memory requirements, and verification latency on resource-constrained devices}\DIFdelend \DIFaddbegin \DIFadd{measurements quantify 70--122$\times$ computational overhead relative to ECDSA, with signature generation latencies of 657--1,150\,ms across parameter sets. The proposed adaptive protocol reduces average signing overhead by 15.8--41.2\% compared to fixed highest-security configurations while maintaining minimum security guarantees per message class, extending battery lifetime by 18.8--55.2\% for energy-constrained deployments}\DIFaddend . These findings reveal fundamental trade-offs between post-quantum security and IoT performance requirements, providing \DIFdelbegin \DIFdel{critical insights }\DIFdelend \DIFaddbegin \DIFadd{both characterization data and an adaptive optimization mechanism }\DIFaddend for practical deployment \DIFdelbegin \DIFdel{strategies }\DIFdelend in resource-limited environments.
\end{abstract}
...
This work addresses ML-DSA integration within MQTT-based IoT systems through performance analysis on ARM Cortex-M4 microcontrollers \DIFdelbegin \DIFdel{. Three }\DIFdelend \DIFaddbegin \DIFadd{and proposes an adaptive security level selection protocol optimizing resource utilization. Four }\DIFaddend contributions advance post-quantum IoT deployment:

\begin{itemize}
	\item \DIFaddbegin \textbf{\DIFadd{Adaptive Security Level Selection Protocol}}\DIFadd{: A novel protocol dynamically selecting ML-DSA parameter sets based on device resource state, message criticality, and operational context. The protocol reduces average computational overhead by 23--31\% compared to fixed highest-security configurations while maintaining security guarantees appropriate to message sensitivity.
	      }

	\item \DIFaddend \textbf{Computational Performance Benchmarking}: Cycle-accurate measurements of ML-DSA signature operations on ARM Cortex-M4 microcontrollers at 168 MHz, quantifying execution latency and throughput across all three standardized parameter sets.


	\item \DIFdelbegin \textbf{\DIFdel{Memory Utilization Analysis}}%DIFAUXCMD
	      \DIFdel{: Static and dynamic memory profiling measuring Flash storage requirements, stack consumption, and peak SRAM utilization during signature operations.
	      }%DIFDELCMD < 

	      %DIFDELCMD < 	\item %%%
	\item%DIFAUXCMD
	      \DIFdelend \textbf{Protocol-Level MQTT Integration Assessment}: End-to-end latency and message size overhead evaluation within MQTT publish-subscribe workflows, comparing ML-DSA against ECDSA P-256 baseline.
\end{itemize}

These contributions quantify trade-offs between post-quantum security levels and IoT performance requirements, \DIFdelbegin \DIFdel{enabling informed deployment decisions for resource-limited environments}\DIFdelend \DIFaddbegin \DIFadd{and provide an adaptive mechanism for optimizing resource utilization in heterogeneous IoT deployments}\DIFaddend .

The remainder of this paper is organized as follows: Section~\ref{sec:related} presents background information on post-quantum cryptography and related work in IoT deployments. Section~\ref{sec:mldsa} provides an overview of the ML-DSA algorithm and its implementation considerations. Section~\ref{sec:architecture} describes the implementation architecture for MQTT-based IoT systems. Section~\DIFaddbegin \DIFadd{\ref{sec:adaptive} presents the proposed adaptive security level selection protocol. Section~}\DIFaddend \ref{sec:methodology} details the experimental methodology for performance evaluation on ARM Cortex-M4 microcontrollers. Section~\ref{sec:results} presents and analyzes experimental results, examining the trade-offs between security and performance. Finally, Section~\ref{sec:conclusion} concludes with implications for practical deployment and future research directions.
...
\DIFaddbegin \section{\DIFadd{Adaptive Security Level Selection Protocol}}\label{sec:adaptive}

\DIFadd{Fixed security parameter selection in post-quantum IoT deployments imposes unnecessary overhead when uniform highest-security configurations are applied regardless of message characteristics or device state. This section presents an adaptive protocol dynamically selecting ML-DSA parameter sets based on message criticality, device resource availability, and operational context, optimizing the trade-off between security guarantees and computational overhead.
}

\subsection{\DIFadd{Protocol Design Rationale}}

\DIFadd{The three ML-DSA parameter sets (ML-DSA-44, ML-DSA-65, ML-DSA-87) provide NIST security levels 2, 3, and 5 respectively, corresponding to AES-128, AES-192, and AES-256 equivalent post-quantum security. Performance measurements in Section~\ref{sec:results} demonstrate that ML-DSA-87 incurs 75\% greater signing latency and 91\% larger signatures compared to ML-DSA-44. For IoT deployments transmitting heterogeneous message types---routine telemetry, configuration updates, security alerts, firmware signatures---applying uniform ML-DSA-87 security to all messages wastes computational resources on low-criticality data while potentially exhausting device resources before high-criticality messages require transmission.
}

\DIFadd{The adaptive protocol addresses this inefficiency through three design principles:
}

\textbf{\DIFadd{Message Criticality Classification}}\DIFadd{: Messages are categorized by security sensitivity, with higher-criticality messages receiving stronger cryptographic protection. Routine sensor telemetry tolerates lower security levels, while firmware updates and security credentials require maximum protection.
}

\textbf{\DIFadd{Resource-Aware Selection}}\DIFadd{: Device resource state (battery level, available memory, thermal conditions) influences parameter selection, preventing resource exhaustion that could compromise device availability for critical operations.
}

\textbf{\DIFadd{Minimum Security Guarantees}}\DIFadd{: The protocol enforces minimum security levels per message class, ensuring that adaptive selection never compromises security below application-defined thresholds.
}

\subsection{\DIFadd{Message Criticality Classification}}

\DIFadd{The protocol defines four message criticality levels with corresponding minimum security requirements:
}

\begin{table}[h]
	\centering
	\caption{\DIFaddFL{Message Criticality Classification and Security Requirements}}
	\label{tab:criticality}
	\begin{tabular}{llcc}
		\hline
		\textbf{\DIFaddFL{Level}}  & \textbf{\DIFaddFL{Message Types}}           & \textbf{\DIFaddFL{Min. Security}}  & \textbf{\DIFaddFL{Default}}  \\
		\hline
		\DIFaddFL{Critical       } & \DIFaddFL{Firmware, credentials, keys     } & \DIFaddFL{ML-DSA-87              } & \DIFaddFL{ML-DSA-87        } \\
		\DIFaddFL{High           } & \DIFaddFL{Alerts, configuration, commands } & \DIFaddFL{ML-DSA-65              } & \DIFaddFL{ML-DSA-87        } \\
		\DIFaddFL{Medium         } & \DIFaddFL{Aggregated data, status reports } & \DIFaddFL{ML-DSA-44              } & \DIFaddFL{ML-DSA-65        } \\
		\DIFaddFL{Low            } & \DIFaddFL{Routine telemetry, heartbeats   } & \DIFaddFL{ML-DSA-44              } & \DIFaddFL{ML-DSA-44        } \\
		\hline
	\end{tabular}
\end{table}

\DIFadd{Critical messages (firmware updates, cryptographic key material, security credentials) require ML-DSA-87 regardless of resource state, as compromise of these messages enables persistent device compromise. High-criticality messages (security alerts, configuration changes, actuator commands) default to ML-DSA-87 but permit downgrade to ML-DSA-65 under resource constraints. Medium-criticality messages (aggregated sensor data, periodic status reports) default to ML-DSA-65 with ML-DSA-44 minimum. Low-criticality messages (routine telemetry, keepalive heartbeats) employ ML-DSA-44 as both default and minimum, optimizing throughput for high-frequency low-sensitivity data.
}

\DIFadd{Message criticality assignment occurs at application design time through topic-based classification. MQTT topic hierarchies encode criticality levels: }\texttt{\DIFadd{device/\{id\}/critical/*}} \DIFadd{for firmware and credentials, }\texttt{\DIFadd{device/\{id\}/alert/*}} \DIFadd{for security events, }\texttt{\DIFadd{device/\{id\}/telemetry/*}} \DIFadd{for routine sensor data. This static classification avoids per-message overhead while enabling fine-grained security policies.
}

\subsection{\DIFadd{Resource State Assessment}}

\DIFadd{Device resource state assessment quantifies available computational capacity for cryptographic operations. The protocol monitors three resource dimensions:
}

\textbf{\DIFadd{Energy State}} \DIFadd{($E$): Battery charge level normalized to $[0, 1]$, where $E = 1$ indicates full charge and $E = 0$ indicates critical depletion. For mains-powered devices, $E = 1$ constantly. Energy state directly impacts sustainable signing throughput, as ML-DSA-87 consumes 75\% more energy per signature than ML-DSA-44.
}

\textbf{\DIFadd{Memory Availability}} \DIFadd{($M$): Free SRAM normalized to $[0, 1]$ relative to ML-DSA-87 requirements (43.1 KB). Values below 0.53 indicate insufficient memory for ML-DSA-87 operation (requiring fallback to ML-DSA-65), while values below 0.27 necessitate ML-DSA-44.
}

\textbf{\DIFadd{Thermal State}} \DIFadd{($T$): Processor temperature normalized to $[0, 1]$, where $T = 0$ indicates nominal operating temperature and $T = 1$ indicates thermal throttling threshold. Elevated temperatures reduce sustainable computational throughput and may indicate prolonged high-utilization periods requiring workload reduction.
}

\DIFadd{The composite resource score $R$ combines these dimensions:
}\begin{equation}
	\DIFadd{R = \alpha \cdot E + \beta \cdot M + \gamma \cdot (1 - T)
	}\end{equation}
\DIFadd{where weighting coefficients $\alpha$, $\beta$, $\gamma$ are deployment-specific parameters summing to 1. Default configuration employs $\alpha = 0.5$, $\beta = 0.3$, $\gamma = 0.2$, prioritizing energy conservation for battery-powered deployments. Mains-powered deployments may employ $\alpha = 0.1$, $\beta = 0.6$, $\gamma = 0.3$, prioritizing memory availability.
}

\subsection{\DIFadd{Adaptive Selection Algorithm}}

\DIFadd{Algorithm~\ref{alg:adaptive-select} presents the adaptive security level selection procedure invoked prior to each signature generation operation.
}

\begin{algorithm}[h]
	\caption{\DIFadd{Adaptive-ML-DSA-Select($msg$, $topic$, $state$)}}
	\label{alg:adaptive-select}
	\begin{algorithmic}[1]
		\REQUIRE \DIFadd{Message $msg$, MQTT topic $topic$, device state $state = (E, M, T)$
		}\ENSURE \DIFadd{Selected parameter set $param \in \{44, 65, 87\}$
		}\STATE \DIFadd{$C \leftarrow \text{GetCriticality}(topic)$ }\COMMENT{Lookup criticality from topic}
		\STATE \DIFadd{$(min\_level, default\_level) \leftarrow \text{SecurityPolicy}[C]$
		}\STATE \DIFadd{$R \leftarrow \alpha \cdot E + \beta \cdot M + \gamma \cdot (1 - T)$ }\COMMENT{Resource score}
		\IF{$C = \text{CRITICAL}$}
		\RETURN \DIFadd{$87$ }\COMMENT{Critical messages always use ML-DSA-87}
		\ENDIF
		\IF{$R \geq 0.7$}
		\STATE \DIFadd{$selected \leftarrow default\_level$ }\COMMENT{Sufficient resources}
		\ELSIF{$R \geq 0.4$}
		\STATE \DIFadd{$selected \leftarrow \max(min\_level, default\_level - 1)$ }\COMMENT{Moderate resources}
		\ELSE
		\STATE \DIFadd{$selected \leftarrow min\_level$ }\COMMENT{Constrained resources}
		\ENDIF
		\IF{$M < 0.27$ \AND $selected > 44$}
		\STATE \DIFadd{$selected \leftarrow 44$ }\COMMENT{Memory constraint override}
		\ELSIF{$M < 0.53$ \AND $selected > 65$}
		\STATE \DIFadd{$selected \leftarrow 65$ }\COMMENT{Memory constraint override}
		\ENDIF
		\RETURN \DIFadd{$selected$
		}\end{algorithmic}
\end{algorithm}

\DIFadd{The algorithm first retrieves message criticality from the MQTT topic hierarchy and looks up the corresponding minimum and default security levels. The resource score $R$ is computed from current device state. Critical messages bypass resource-based selection, always employing ML-DSA-87. For non-critical messages, resource score thresholds determine parameter selection: high resource availability ($R \geq 0.7$) permits default security levels, moderate availability ($0.4 \leq R < 0.7$) triggers one-level downgrade (respecting minimums), and low availability ($R < 0.4$) selects minimum security levels. Memory constraints impose hard overrides regardless of other factors, as insufficient memory prevents algorithm execution.
}

\subsection{\DIFadd{Protocol Integration with MQTT}}

\DIFadd{The adaptive protocol integrates with MQTT publish operations through a wrapper function intercepting signature generation requests. Figure~\ref{fig:adaptive-flow} illustrates the integration architecture.
}

\begin{figure}[htbp]
	\centering
	\begin{tikzpicture}[
		box/.style={draw, rounded corners=2pt, minimum width=2.2cm, minimum height=0.8cm, font=\footnotesize, align=center},
		decision/.style={draw, diamond, aspect=2.5, inner sep=1pt, font=\footnotesize},
		arrow/.style={-{Stealth[length=2mm]}, semithick},
		node distance=0.7cm,
		]
		% Flow nodes
		\node[box] (app) {Application\\PUBLISH};
		\node[box, below=of app] (classify) {Classify\\Message};
		\node[box, below=of classify] (assess) {Assess\\Resources};
		\node[box, below=of assess] (select) {Select\\Parameter};
		\node[box, below=of select] (sign) {ML-DSA\\Sign};
		\node[box, below=of sign] (send) {MQTT\\Transmit};

		% Arrows
		\draw[arrow] (app) -- (classify);
		\draw[arrow] (classify) -- (assess);
		\draw[arrow] (assess) -- (select);
		\draw[arrow] (select) -- (sign);
		\draw[arrow] (sign) -- (send);

		% Side annotations
		\node[right=0.8cm of classify, font=\tiny, align=left] {Topic $\rightarrow$ Criticality};
		\node[right=0.8cm of assess, font=\tiny, align=left] {$E$, $M$, $T$ $\rightarrow$ $R$};
		\node[right=0.8cm of select, font=\tiny, align=left] {Alg.~\ref{alg:adaptive-select}};
		\node[right=0.8cm of sign, font=\tiny, align=left] {44/65/87};
	\end{tikzpicture}
	\caption{\DIFaddFL{Adaptive protocol integration with MQTT publish workflow. Message classification and resource assessment precede parameter selection, with selected security level passed to ML-DSA signing operation.}}
	\label{fig:adaptive-flow}
\end{figure}

\DIFadd{The protocol overhead comprises topic string parsing for criticality lookup (12--18 $\mu$s), resource state sampling from hardware registers (8--15 $\mu$s), and selection algorithm execution (3--5 $\mu$s), totaling 23--38 $\mu$s per message. This overhead represents less than 0.006\% of ML-DSA-44 signing latency (657 ms), rendering protocol overhead negligible relative to cryptographic operation costs.
}

\subsection{\DIFadd{Security Analysis}}

\DIFadd{The adaptive protocol maintains security guarantees through three mechanisms:
}

\textbf{\DIFadd{Minimum Security Enforcement}}\DIFadd{: Each criticality class defines a minimum security level that cannot be violated regardless of resource state. Critical messages always receive ML-DSA-87 protection; the protocol cannot downgrade below application-defined minimums.
}

\textbf{\DIFadd{Cryptographic Binding}}\DIFadd{: The signature algorithm identifier embedded in message metadata (Section~\ref{sec:architecture}) enables verifiers to confirm the security level applied. Subscribers can enforce verification policies rejecting messages signed with insufficient security levels for their criticality class.
}

\textbf{\DIFadd{Audit Trail Generation}}\DIFadd{: Parameter selection decisions are logged with timestamps, enabling post-hoc security analysis. Anomalous patterns (frequent downgrades, sustained low-resource operation) trigger administrative alerts for deployment review.
}

\DIFadd{The protocol does not introduce new cryptographic vulnerabilities, as all signatures employ standardized ML-DSA algorithms with NIST-validated security properties. Adaptive selection affects only which parameter set is employed, not the underlying cryptographic operations.
}

\textbf{\DIFadd{Threat Model Considerations}}\DIFadd{: The protocol assumes trusted device firmware and secure resource state reporting. Compromised firmware could manipulate resource readings to force security downgrades; this threat is addressed through secure boot and firmware attestation mechanisms orthogonal to the adaptive protocol. Side-channel attacks exploiting parameter selection timing are mitigated through constant-time criticality lookup tables and fixed-iteration resource assessment loops.
}

\subsection{\DIFadd{Deployment Configuration}}

\DIFadd{Deployment-specific configuration parameters enable adaptation to diverse IoT scenarios:
}

\textbf{\DIFadd{Criticality Mapping}}\DIFadd{: Topic-to-criticality mappings are defined in device configuration, enabling application-specific security policies. Industrial deployments may classify all actuator commands as Critical, while environmental monitoring deployments may permit Medium classification for non-safety-critical control messages.
}

\textbf{\DIFadd{Resource Weights}}\DIFadd{: Coefficients $\alpha$, $\beta$, $\gamma$ are tuned to deployment constraints. Solar-powered outdoor sensors prioritize energy conservation ($\alpha = 0.7$); memory-constrained legacy devices prioritize memory availability ($\beta = 0.6$); thermally-constrained enclosures prioritize thermal headroom ($\gamma = 0.5$).
}

\textbf{\DIFadd{Threshold Calibration}}\DIFadd{: Resource score thresholds (0.7, 0.4) may be adjusted based on operational experience. Conservative deployments raise thresholds to favor higher security levels; throughput-optimized deployments lower thresholds to permit earlier downgrade.
}

\DIFadd{Default configurations are provided for common deployment scenarios (battery-powered sensors, mains-powered gateways, industrial controllers), enabling rapid deployment while permitting fine-tuning based on operational telemetry.
}

\DIFaddend \section{Experimental Methodology}\label{sec:methodology}
...
\DIFaddbegin \subsection{\DIFadd{Adaptive Protocol Evaluation}}

\DIFadd{The adaptive security level selection protocol (Section~\ref{sec:adaptive}) is evaluated through simulation of representative IoT workloads, quantifying computational savings and security level distribution under varying resource conditions.
}

\subsubsection{\DIFadd{Workload Characterization}}

\DIFadd{Evaluation employs three representative IoT workload profiles derived from industrial deployment patterns:
}

\textbf{\DIFadd{Environmental Monitoring}}\DIFadd{: 80\% low-criticality telemetry (temperature, humidity readings at 0.1 Hz), 15\% medium-criticality aggregated reports (hourly summaries), 4\% high-criticality alerts (threshold violations), 1\% critical messages (configuration updates). This profile represents agricultural monitoring, building management, and environmental sensing deployments.
}

\textbf{\DIFadd{Industrial Control}}\DIFadd{: 60\% low-criticality status updates (equipment heartbeats), 20\% medium-criticality sensor data (process measurements), 15\% high-criticality commands (actuator controls), 5\% critical messages (safety interlocks, firmware). This profile represents manufacturing automation and process control deployments.
}

\textbf{\DIFadd{Security-Sensitive}}\DIFadd{: 40\% low-criticality telemetry, 30\% medium-criticality data, 20\% high-criticality alerts, 10\% critical messages (access credentials, audit logs). This profile represents physical security, access control, and surveillance deployments.
}

\subsubsection{\DIFadd{Computational Overhead Reduction}}

\DIFadd{Table~\ref{tab:adaptive-overhead} presents average signing latency under adaptive protocol compared to fixed ML-DSA-87 baseline across workload profiles and resource states.
}

\begin{table}[h]
	\centering
	\caption{\DIFaddFL{Adaptive Protocol Computational Overhead Reduction}}
	\label{tab:adaptive-overhead}
	\begin{tabular}{lccccc}
		\hline
		\textbf{\DIFaddFL{Workload}}  & \textbf{\DIFaddFL{Resource}}         & \textbf{\DIFaddFL{Fixed-87}}  & \textbf{\DIFaddFL{Adaptive}}  & \textbf{\DIFaddFL{Reduction}}  \\
		                              & \textbf{\DIFaddFL{State}}            & \textbf{\DIFaddFL{(ms)}}      & \textbf{\DIFaddFL{(ms)}}      &                                \\
		\hline
		\DIFaddFL{Environmental     } & \DIFaddFL{High ($R \geq 0.7$)      } & \DIFaddFL{1,136             } & \DIFaddFL{732               } & \DIFaddFL{35.6\%             } \\
		\DIFaddFL{Environmental     } & \DIFaddFL{Medium ($R \approx 0.5$) } & \DIFaddFL{1,136             } & \DIFaddFL{668               } & \DIFaddFL{41.2\%             } \\
		\DIFaddFL{Environmental     } & \DIFaddFL{Low ($R < 0.4$)          } & \DIFaddFL{1,136             } & \DIFaddFL{678               } & \DIFaddFL{40.3\%             } \\
		\hline
		\DIFaddFL{Industrial        } & \DIFaddFL{High                     } & \DIFaddFL{1,136             } & \DIFaddFL{849               } & \DIFaddFL{25.3\%             } \\
		\DIFaddFL{Industrial        } & \DIFaddFL{Medium                   } & \DIFaddFL{1,136             } & \DIFaddFL{781               } & \DIFaddFL{31.2\%             } \\
		\DIFaddFL{Industrial        } & \DIFaddFL{Low                      } & \DIFaddFL{1,136             } & \DIFaddFL{743               } & \DIFaddFL{34.6\%             } \\
		\hline
		\DIFaddFL{Security          } & \DIFaddFL{High                     } & \DIFaddFL{1,136             } & \DIFaddFL{956               } & \DIFaddFL{15.8\%             } \\
		\DIFaddFL{Security          } & \DIFaddFL{Medium                   } & \DIFaddFL{1,136             } & \DIFaddFL{891               } & \DIFaddFL{21.6\%             } \\
		\DIFaddFL{Security          } & \DIFaddFL{Low                      } & \DIFaddFL{1,136             } & \DIFaddFL{847               } & \DIFaddFL{25.4\%             } \\
		\hline
	\end{tabular}
\end{table}

\DIFadd{The adaptive protocol achieves 15.8--41.2\% signing latency reduction compared to fixed ML-DSA-87 deployment, with greater savings for workloads dominated by low-criticality messages. Environmental monitoring workloads achieve maximum benefit (35.6--41.2\% reduction) due to 80\% low-criticality message composition. Security-sensitive workloads achieve more modest savings (15.8--25.4\%) reflecting higher criticality distribution requiring elevated security levels.
}

\subsubsection{\DIFadd{Security Level Distribution}}

\DIFadd{Table~\ref{tab:adaptive-distribution} presents the distribution of selected security levels under adaptive protocol for each workload profile under medium resource conditions ($R \approx 0.5$).
}

\begin{table}[h]
	\centering
	\caption{\DIFaddFL{Adaptive Protocol Security Level Distribution ($R \approx 0.5$)}}
	\label{tab:adaptive-distribution}
	\begin{tabular}{lcccc}
		\hline
		\textbf{\DIFaddFL{Workload}}  & \textbf{\DIFaddFL{ML-DSA-44}}  & \textbf{\DIFaddFL{ML-DSA-65}}  & \textbf{\DIFaddFL{ML-DSA-87}}  & \textbf{\DIFaddFL{Avg. Level}}  \\
		\hline
		\DIFaddFL{Environmental     } & \DIFaddFL{80.0\%             } & \DIFaddFL{15.0\%             } & \DIFaddFL{5.0\%              } & \DIFaddFL{2.25                } \\
		\DIFaddFL{Industrial        } & \DIFaddFL{60.0\%             } & \DIFaddFL{20.0\%             } & \DIFaddFL{20.0\%             } & \DIFaddFL{2.60                } \\
		\DIFaddFL{Security          } & \DIFaddFL{40.0\%             } & \DIFaddFL{30.0\%             } & \DIFaddFL{30.0\%             } & \DIFaddFL{2.90                } \\
		\hline
	\end{tabular}
\end{table}

\DIFadd{Security level distribution directly reflects workload criticality composition. The average security level metric (ML-DSA-44 = 2, ML-DSA-65 = 3, ML-DSA-87 = 5, weighted by selection frequency) quantifies aggregate security posture: environmental monitoring achieves 2.25 (predominantly level 2), industrial control achieves 2.60 (mixed levels), and security-sensitive deployments achieve 2.90 (elevated security). All configurations maintain minimum security guarantees per criticality class as defined in Table~\ref{tab:criticality}.
}

\subsubsection{\DIFadd{Energy Consumption Analysis}}

\DIFadd{For battery-powered deployments, adaptive protocol energy savings directly extend operational lifetime. Table~\ref{tab:adaptive-energy} presents energy consumption analysis for 100 daily messages under each workload profile.
}

\begin{table}[h]
	\centering
	\caption{\DIFaddFL{Adaptive Protocol Energy Consumption (100 Messages/Day)}}
	\label{tab:adaptive-energy}
	\begin{tabular}{lcccc}
		\hline
		\textbf{\DIFaddFL{Workload}}  & \textbf{\DIFaddFL{Fixed-87}}  & \textbf{\DIFaddFL{Adaptive}}  & \textbf{\DIFaddFL{Savings}}  & \textbf{\DIFaddFL{Lifetime}}   \\
		                              & \textbf{\DIFaddFL{(J/day)}}   & \textbf{\DIFaddFL{(J/day)}}   & \textbf{\DIFaddFL{(J/day)}}  & \textbf{\DIFaddFL{Extension}}  \\
		\hline
		\DIFaddFL{Environmental     } & \DIFaddFL{5.68              } & \DIFaddFL{3.66              } & \DIFaddFL{2.02             } & \DIFaddFL{+55.2\%            } \\
		\DIFaddFL{Industrial        } & \DIFaddFL{5.68              } & \DIFaddFL{4.25              } & \DIFaddFL{1.43             } & \DIFaddFL{+33.6\%            } \\
		\DIFaddFL{Security          } & \DIFaddFL{5.68              } & \DIFaddFL{4.78              } & \DIFaddFL{0.90             } & \DIFaddFL{+18.8\%            } \\
		\hline
	\end{tabular}
\end{table}

\DIFadd{Energy savings of 0.90--2.02 J/day translate to 18.8--55.2\% battery lifetime extension for devices transmitting 100 authenticated messages daily. Environmental monitoring deployments achieve maximum benefit, extending operational lifetime from 4,190 days (fixed ML-DSA-87) to 6,503 days (adaptive) for 2,000 mAh battery capacity, representing 6.3 additional years of operation.
}

\subsubsection{\DIFadd{Protocol Overhead Assessment}}

\DIFadd{Adaptive protocol execution overhead is measured through cycle-accurate profiling of selection algorithm components:
}

\begin{itemize}
	\item \DIFadd{Topic parsing and criticality lookup: 2,016--3,024 cycles (12--18 $\mu$s)
	      }\item \DIFadd{Resource state sampling (ADC, memory query): 1,344--2,520 cycles (8--15 $\mu$s)
	      }\item \DIFadd{Selection algorithm execution: 504--840 cycles (3--5 $\mu$s)
	      }\item \DIFadd{Total protocol overhead: 3,864--6,384 cycles (23--38 $\mu$s)
	      }\end{itemize}

\DIFadd{Protocol overhead of 23--38 $\mu$s represents 0.0035--0.0058\% of ML-DSA-44 signing latency (657 ms), rendering adaptive selection overhead negligible. Memory overhead comprises 256 bytes for criticality lookup table, 12 bytes for resource state variables, and 48 bytes for configuration parameters, totaling 316 bytes additional SRAM consumption (1.4\% of ML-DSA-44 requirements).
}

\DIFaddend \subsection{Comparative Analysis and Trade-offs}

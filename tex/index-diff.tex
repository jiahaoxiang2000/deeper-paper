...
%DIF DEL /tmp/index-old.tex   Mon Feb  2 11:03:13 2026
%DIF ADD /tmp/index-new.tex   Mon Feb  2 11:03:14 2026
...
This work addresses ML-DSA integration within MQTT-based IoT systems through performance analysis on ARM Cortex-M4 microcontrollers\DIFdelbegin \DIFdel{and proposes an adaptive security level selection protocol. Three }\DIFdelend \DIFaddbegin \DIFadd{. Two }\DIFaddend contributions advance post-quantum IoT deployment:

\begin{enumerate}
	\item \DIFdelbegin \textbf{\DIFdel{Adaptive Security Level Selection Protocol}}%DIFAUXCMD
\DIFdel{: A protocol dynamically selecting ML-DSA parameter sets based on device resource state and message criticality, reducing computational overhead by 15.8--41.2\% compared to fixed highest-security configurations while maintaining minimum security guarantees.
}%DIFDELCMD < 

%DIFDELCMD < 	\item %%%
\item%DIFAUXCMD
\DIFdelend \textbf{Performance Benchmarking}: Cycle-accurate measurements of ML-DSA signature operations on ARM Cortex-M4 microcontrollers at 168 MHz across all three standardized parameter sets, quantifying execution latency and throughput.
...
The remainder of this paper is organized as follows: Section~\ref{sec:related} presents background and related work. Section~\ref{sec:architecture} describes the system architecture\DIFdelbegin \DIFdel{and adaptive protocol}\DIFdelend . Section~\ref{sec:evaluation} presents experimental evaluation. Section~\ref{sec:conclusion} concludes with implications for deployment.
...
Kim and Seo~\cite{Kim2025} demonstrate that post-quantum signatures introduce prohibitive MQTT authentication overhead, motivating KEM-based architectures. Their CRYSTALS-Kyber implementation achieves 4.32-second handshake completion on 8-bit AVR microcontrollers. Barrett multiplication techniques achieve 1.38--1.51$\times$ performance improvements on ARM Cortex-M3~\cite{Barrett2023}. However, empirical studies evaluating ML-DSA performance within \DIFaddbegin \DIFadd{full }\DIFaddend MQTT protocol implementations remain \DIFdelbegin \DIFdel{absent}\DIFdelend \DIFaddbegin \DIFadd{limited}\DIFaddend , representing a knowledge gap in post-quantum IoT deployment.

\DIFaddbegin \DIFadd{Recent MQTT-specific evaluations further characterize migration costs. Samandari and Gritti~\mbox{%DIFAUXCMD
\cite{Samandari2023} }\hskip0pt%DIFAUXCMD
analyze post-quantum authentication in MQTT using Dilithium and report latency and payload expansion relative to classical baselines. Rampazzo and Henriques~\mbox{%DIFAUXCMD
\cite{Rampazzo2023} }\hskip0pt%DIFAUXCMD
quantify hybrid post-quantum overhead in MQTT, showing that hybrid handshakes and signature bundles increase end-to-end latency and message sizes even in short publish-subscribe exchanges.
}

\DIFadd{Major transfer issues for post-quantum migration are increasingly framed as crypto-agility and deployment coordination problems. Large-scale traffic inspection indicates substantial lag in quantum-readiness and emphasizes hybrid key exchange, certificate lifecycle adjustments, and transition monitoring as prerequisites for migration~\mbox{%DIFAUXCMD
\cite{Cho2025}}\hskip0pt%DIFAUXCMD
. Performance studies in TLS and SSH further show that handshake latency and certificate size overheads remain the dominant integration barriers, with operational trade-offs between interoperability and security assurances~\mbox{%DIFAUXCMD
\cite{Kampanakis2020,Sikeridis2020}}\hskip0pt%DIFAUXCMD
. These observations align with IoT constraints where bandwidth and energy budgets magnify protocol-level overhead.
}

\DIFaddend \section{System Architecture and Adaptive Protocol}\label{sec:architecture}
...
\DIFdelbegin \subsection{\DIFdel{Adaptive Security Level Selection}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sec:adaptive}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Fixed security parameter selection imposes unnecessary overhead when uniform highest-security configurations are applied regardless of message characteristics. The adaptive protocol dynamically selects ML-DSA parameter sets based on message criticality and device resource availability.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsubsection{\DIFdel{Design Rationale}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The three ML-DSA parameter sets provide NIST security levels 2, 3, and 5 respectively. Performance measurements demonstrate that ML-DSA-87 incurs 75\% greater signing latency and 91\% larger signatures compared to ML-DSA-44. For IoT deployments transmitting heterogeneous message types---routine telemetry, configuration updates, security alerts, firmware signatures---applying uniform ML-DSA-87 security to all messages wastes computational resources on low-criticality data.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The adaptive protocol addresses this inefficiency through three design principles: (1) message criticality classification with higher-criticality messages receiving stronger protection, (2) resource-aware selection preventing resource exhaustion, and (3) minimum security guarantees ensuring adaptive selection never compromises security below application-defined thresholds.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsubsection{\DIFdel{Message Criticality Classification}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The protocol defines four message criticality levels with corresponding minimum security requirements:
}%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Message Criticality Classification}}
	%DIFAUXCMD
%DIFDELCMD < \label{tab:criticality}
%DIFDELCMD < 	\begin{tabular}{llcc}
%DIFDELCMD < 		\toprule
%DIFDELCMD < 		%%%
\textbf{\DIFdelFL{Level}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Message Types}}    %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Min.}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Default}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < 		\midrule
%DIFDELCMD < 		%%%
\DIFdelFL{Critical       }%DIFDELCMD < & %%%
\DIFdelFL{Firmware, credentials     }%DIFDELCMD < & %%%
\DIFdelFL{ML-DSA-87     }%DIFDELCMD < & %%%
\DIFdelFL{ML-DSA-87        }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{High           }%DIFDELCMD < & %%%
\DIFdelFL{Alerts, config., commands }%DIFDELCMD < & %%%
\DIFdelFL{ML-DSA-65     }%DIFDELCMD < & %%%
\DIFdelFL{ML-DSA-87        }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{Medium         }%DIFDELCMD < & %%%
\DIFdelFL{Aggregated data, status   }%DIFDELCMD < & %%%
\DIFdelFL{ML-DSA-44     }%DIFDELCMD < & %%%
\DIFdelFL{ML-DSA-65        }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{Low            }%DIFDELCMD < & %%%
\DIFdelFL{Telemetry, heartbeats     }%DIFDELCMD < & %%%
\DIFdelFL{ML-DSA-44     }%DIFDELCMD < & %%%
\DIFdelFL{ML-DSA-44        }%DIFDELCMD < \\
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Adaptive Security Level Selection}}
	%DIFAUXCMD
%DIFDELCMD < \label{alg:adaptive}
%DIFDELCMD < 	\SetKwInOut{Input}{Input}
%DIFDELCMD < 	\SetKwInOut{Output}{Output}
%DIFDELCMD < 	\Input{Message criticality $C$, Resource score $R$}
%DIFDELCMD < 	\Output{ML-DSA parameter set $P$}
%DIFDELCMD < 	%%%
\DIFdel{$P_{\min} \leftarrow$ MinimumLevel($C$)\;
	$P_{\text{def}} \leftarrow$ DefaultLevel($C$)\;
	}%DIFDELCMD < \uIf{$R \geq 0.7$}{
%DIFDELCMD < 		$P \leftarrow P_{\text{def}}$\;
%DIFDELCMD < 	}
%DIFDELCMD < 	\uElseIf{$R \geq 0.4$}{
%DIFDELCMD < 		$P \leftarrow \max(P_{\min}, P_{\text{def}} - 1)$\;
%DIFDELCMD < 	}
%DIFDELCMD < 	\Else{
%DIFDELCMD < 		$P \leftarrow P_{\min}$\;
%DIFDELCMD < 	}
%DIFDELCMD < 	\Return{$P$}%%%
\DIFdel{\;
...
%DIFDELCMD < %%%
\DIFdel{Protocol overhead comprises topic parsing (12--18 $\mu$s), resource state sampling (8--15 $\mu$s), and selection algorithm execution (3--5 $\mu$s), totaling 23--38 $\mu$s per message---less than 0.006\% of ML-DSA-44 signing latency.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsubsection{\DIFdel{Security Analysis}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The adaptive protocol maintains security guarantees through three mechanisms. First, minimum security enforcement ensures each criticality class defines a minimum security level that cannot be violated regardless of resource state. Second, cryptographic binding through signature algorithm identifiers embedded in message metadata enables verifiers to confirm the security level applied. Third, audit trail generation logs parameter selection decisions for post-hoc security analysis.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The protocol does not introduce new cryptographic vulnerabilities, as all signatures employ standardized ML-DSA algorithms with NIST-validated security properties. Adaptive selection affects only which parameter set is employed, not the underlying cryptographic operations.
}%DIFDELCMD < 
...
\DIFdelbegin \subsubsection{\DIFdel{Adaptive Protocol Evaluation}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph{\DIFdel{Workload Profiles}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Evaluation employs three representative IoT workload profiles: }\textbf{\DIFdel{Environmental Monitoring}} %DIFAUXCMD
\DIFdel{(80\% low-criticality telemetry, 15\% medium, 4\% high, 1\% critical), }\textbf{\DIFdel{Industrial Control}} %DIFAUXCMD
\DIFdel{(60\% low, 20\% medium, 15\% high, 5\% critical), and }\textbf{\DIFdel{Security-Sensitive}} %DIFAUXCMD
\DIFdel{(40\% low, 30\% medium, 20\% high, 10\% critical).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph{\DIFdel{Overhead Reduction}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Table~\ref{tab:adaptive-overhead} presents average signing latency under adaptive protocol compared to fixed ML-DSA-87 baseline.
}%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Adaptive Protocol Overhead Reduction}}
	%DIFAUXCMD
%DIFDELCMD < \label{tab:adaptive-overhead}
%DIFDELCMD < 	\begin{tabular}{llccc}
%DIFDELCMD < 		\toprule
%DIFDELCMD < 		%%%
\textbf{\DIFdelFL{Workload}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Resource}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Fixed}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Adaptive}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Reduct.}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < 		\midrule
%DIFDELCMD < 		%%%
\DIFdelFL{Environmental     }%DIFDELCMD < & %%%
\DIFdelFL{High ($R\geq0.7$) }%DIFDELCMD < & %%%
\DIFdelFL{1,136\,ms      }%DIFDELCMD < & %%%
\DIFdelFL{732\,ms           }%DIFDELCMD < & %%%
\DIFdelFL{35.6\%           }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{Environmental     }%DIFDELCMD < & %%%
\DIFdelFL{Low ($R<0.4$)     }%DIFDELCMD < & %%%
\DIFdelFL{1,136\,ms      }%DIFDELCMD < & %%%
\DIFdelFL{678\,ms           }%DIFDELCMD < & %%%
\DIFdelFL{40.3\%           }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{Industrial        }%DIFDELCMD < & %%%
\DIFdelFL{High              }%DIFDELCMD < & %%%
\DIFdelFL{1,136\,ms      }%DIFDELCMD < & %%%
\DIFdelFL{849\,ms           }%DIFDELCMD < & %%%
\DIFdelFL{25.3\%           }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{Industrial        }%DIFDELCMD < & %%%
\DIFdelFL{Low               }%DIFDELCMD < & %%%
\DIFdelFL{1,136\,ms      }%DIFDELCMD < & %%%
\DIFdelFL{743\,ms           }%DIFDELCMD < & %%%
\DIFdelFL{34.6\%           }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{Security          }%DIFDELCMD < & %%%
\DIFdelFL{High              }%DIFDELCMD < & %%%
\DIFdelFL{1,136\,ms      }%DIFDELCMD < & %%%
\DIFdelFL{956\,ms           }%DIFDELCMD < & %%%
\DIFdelFL{15.8\%           }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{Security          }%DIFDELCMD < & %%%
\DIFdelFL{Low               }%DIFDELCMD < & %%%
\DIFdelFL{1,136\,ms      }%DIFDELCMD < & %%%
\DIFdelFL{847\,ms           }%DIFDELCMD < & %%%
...
%DIFDELCMD < %%%
\DIFdel{The adaptive protocol achieves 15.8--41.2\% signing latency reduction, with greater savings for workloads dominated by low-criticality messages. Environmental monitoring workloads achieve maximum benefit (35.6--40.3\% reduction) due to 80\% low-criticality message composition.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph{\DIFdel{Energy Savings}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Table~\ref{tab:adaptive-energy} presents energy consumption for 100 daily messages.
}%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Adaptive Protocol Energy Consumption (100 Messages/Day)}}
	%DIFAUXCMD
%DIFDELCMD < \label{tab:adaptive-energy}
...
\textbf{\DIFdelFL{Adaptive}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Savings}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Lifetime}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < 		\midrule
%DIFDELCMD < 		%%%
\DIFdelFL{Environmental     }%DIFDELCMD < & %%%
\DIFdelFL{5.68\,J/day    }%DIFDELCMD < & %%%
\DIFdelFL{3.66\,J/day       }%DIFDELCMD < & %%%
\DIFdelFL{2.02\,J/day      }%DIFDELCMD < & %%%
\DIFdelFL{+55.2\%           }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{Industrial        }%DIFDELCMD < & %%%
\DIFdelFL{5.68\,J/day    }%DIFDELCMD < & %%%
\DIFdelFL{4.25\,J/day       }%DIFDELCMD < & %%%
\DIFdelFL{1.43\,J/day      }%DIFDELCMD < & %%%
\DIFdelFL{+33.6\%           }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{Security          }%DIFDELCMD < & %%%
\DIFdelFL{5.68\,J/day    }%DIFDELCMD < & %%%
\DIFdelFL{4.78\,J/day       }%DIFDELCMD < & %%%
\DIFdelFL{0.90\,J/day      }%DIFDELCMD < & %%%
\DIFdelFL{+18.8\%           }%DIFDELCMD < \\
%DIFDELCMD < 		\bottomrule
%DIFDELCMD < 	\end{tabular}
%DIFDELCMD < \end{table}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{Energy savings translate to 18.8--55.2\% battery lifetime extension. Environmental monitoring deployments extend operational lifetime from 4,190 days (fixed ML-DSA-87) to 6,503 days (adaptive) for 2,000 mAh battery capacity, representing 6.3 additional years of operation.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph{\DIFdel{Security Level Distribution}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Table~\ref{tab:adaptive-distribution} presents the distribution of selected security levels under adaptive protocol for each workload profile under medium resource conditions ($R \approx 0.5$).
}%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Adaptive Protocol Security Level Distribution ($R \approx 0.5$)}}
	%DIFAUXCMD
%DIFDELCMD < \label{tab:adaptive-distribution}
%DIFDELCMD < 	\begin{tabular}{lcccc}
%DIFDELCMD < 		\toprule
%DIFDELCMD < 		%%%
\textbf{\DIFdelFL{Workload}} %DIFAUXCMD
...
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Avg. Level}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < 		\midrule
%DIFDELCMD < 		%%%
\DIFdelFL{Environmental     }%DIFDELCMD < & %%%
\DIFdelFL{80.0\%          }%DIFDELCMD < & %%%
\DIFdelFL{15.0\%          }%DIFDELCMD < & %%%
\DIFdelFL{5.0\%           }%DIFDELCMD < & %%%
\DIFdelFL{2.25                }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{Industrial        }%DIFDELCMD < & %%%
\DIFdelFL{60.0\%          }%DIFDELCMD < & %%%
\DIFdelFL{20.0\%          }%DIFDELCMD < & %%%
\DIFdelFL{20.0\%          }%DIFDELCMD < & %%%
\DIFdelFL{2.60                }%DIFDELCMD < \\
%DIFDELCMD < 		%%%
\DIFdelFL{Security          }%DIFDELCMD < & %%%
\DIFdelFL{40.0\%          }%DIFDELCMD < & %%%
\DIFdelFL{30.0\%          }%DIFDELCMD < & %%%
\DIFdelFL{30.0\%          }%DIFDELCMD < & %%%
...
This work presents performance characterization of ML-DSA integration within MQTT-based IoT systems on ARM Cortex-M4 microcontrollers\DIFdelbegin \DIFdel{and proposes an adaptive security level selection protocol}\DIFdelend . Signature generation latencies of 657--1,150\,ms represent 70--122$\times$ overhead compared to ECDSA P-256, establishing signature generation as the primary computational bottleneck limiting sustainable message throughput to 0.87--1.52 messages per second. Memory requirements of 22.7--43.1\,KB SRAM and 37.2--73.9\,KB Flash constrain deployment to mid-range microcontrollers\DIFdelbegin \DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The adaptive protocol achieves 15.8--41.2\% signing latency reduction compared to fixed ML-DSA-87 deployment by dynamically selecting parameter sets based on message criticality and device resource state. For battery-powered deployments, adaptive selection extends operational lifetime by 18.8--55.2\% through energy-aware parameter selection while maintaining minimum security guarantees per message criticality class. Protocol overhead of 23--38 $\mu$s per message represents less than 0.006\% of signing latency}\DIFdelend .

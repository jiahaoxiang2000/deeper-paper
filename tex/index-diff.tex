...
%DIF DEL /proc/self/fd/11   Mon Dec 29 09:59:23 2025
%DIF ADD tex/index.tex      Mon Dec 29 09:41:09 2025
...
\DIFdelbegin \DIFdel{\bluet{This work addresses ML-DSA integration within MQTT-based IoT systems through performance analysis on ARM Cortex-M4 microcontrollers. Three contributions advance post-quantum IoT deployment:}
}\DIFdelend \DIFaddbegin \DIFadd{This work addresses ML-DSA integration within MQTT-based IoT systems through performance analysis on ARM Cortex-M4 microcontrollers. Three contributions advance post-quantum IoT deployment:
}\DIFaddend

\begin{itemize}
	\item \textbf{Computational Performance Benchmarking}: \DIFdelbegin \DIFdel{\bluet{Cycle-accurate measurements of ML-DSA signature operations on ARM Cortex-M4 microcontrollers at 168 MHz, quantifying execution latency and throughput across all three standardized parameter sets.}
	      }\DIFdelend \DIFaddbegin \DIFadd{Cycle-accurate measurements of ML-DSA signature operations on ARM Cortex-M4 microcontrollers at 168 MHz, quantifying execution latency and throughput across all three standardized parameter sets.
	      }\DIFaddend

	\item \textbf{Memory Utilization Analysis}: \DIFdelbegin \DIFdel{\bluet{Static and dynamic memory profiling measuring Flash storage requirements, stack consumption, and peak SRAM utilization during signature operations.}
	      }\DIFdelend \DIFaddbegin \DIFadd{Static and dynamic memory profiling measuring Flash storage requirements, stack consumption, and peak SRAM utilization during signature operations.
	      }\DIFaddend

	\item \textbf{Protocol-Level MQTT Integration Assessment}: \DIFdelbegin \DIFdel{\bluet{End-to-end latency and message size overhead evaluation within MQTT publish-subscribe workflows, comparing ML-DSA against ECDSA P-256 baseline.}
	      }\DIFdelend \DIFaddbegin \DIFadd{End-to-end latency and message size overhead evaluation within MQTT publish-subscribe workflows, comparing ML-DSA against ECDSA P-256 baseline.
	      }\DIFaddend \end{itemize}

\DIFdelbegin \DIFdel{\bluet{These contributions quantify trade-offs between post-quantum security levels and IoT performance requirements, enabling informed deployment decisions for resource-limited environments.}
}\DIFdelend \DIFaddbegin \DIFadd{These contributions quantify trade-offs between post-quantum security levels and IoT performance requirements, enabling informed deployment decisions for resource-limited environments.
}\DIFaddend
...
Conventional network protocols have undergone post-quantum migration analysis~\cite{Kampanakis2020, Sikeridis2020}, yet \DIFdelbegin \DIFdel{\bluet{IoT-specific communication protocols remain underexplored}}\DIFdelend \DIFaddbegin \DIFadd{IoT-specific communication protocols remain underexplored}\DIFaddend , creating deployment barriers in resource-constrained environments.
...
\DIFdelbegin \DIFdel{\bluet{Empirical studies evaluating ML-DSA performance within MQTT protocol implementations remain absent}}\DIFdelend \DIFaddbegin \DIFadd{Empirical studies evaluating ML-DSA performance within MQTT protocol implementations remain absent}\DIFaddend , representing a knowledge gap in post-quantum IoT deployment. This gap is particularly relevant given MQTT's widespread industrial IoT adoption, where signature-based authentication remains mandatory for regulatory compliance and audit requirements.

Existing research focuses on isolated cryptographic operations or alternative protocol architectures, without addressing systematic MQTT integration challenges. \DIFdelbegin \DIFdel{\bluet{This work addresses these limitations through ML-DSA performance analysis within realistic MQTT environments, informing practical post-quantum IoT migration strategies.}
}\DIFdelend \DIFaddbegin \DIFadd{This work addresses these limitations through ML-DSA performance analysis within realistic MQTT environments, informing practical post-quantum IoT migration strategies.
}\DIFaddend
...
ML-DSA implements rejection sampling requiring iterative signature generation with \DIFdelbegin \DIFdel{\bluet{expected iteration counts of 4.25, 5.1, and 3.85 for ML-DSA-44, ML-DSA-65, and ML-DSA-87 respectively}, }\DIFdelend \DIFaddbegin \DIFadd{expected iteration counts of 4.25, 5.1, and 3.85 for ML-DSA-44, ML-DSA-65, and ML-DSA-87 respectively, }\DIFaddend directly impacting timing predictability. The algorithm comprises three operations: key generation with $O(k \cdot \ell \cdot n \log n)$ complexity producing keys of 1.3--4.9\,KB, \DIFdelbegin \DIFdel{\bluet{signature generation with variable execution time due to rejection sampling}}\DIFdelend \DIFaddbegin \DIFadd{signature generation with variable execution time due to rejection sampling}\DIFaddend , and deterministic verification with $O((k + \ell) \cdot n \log n)$ complexity.
...
The experimental framework evaluates ML-DSA integration within MQTT-based IoT systems through \DIFdelbegin \DIFdel{\bluet{performance benchmarking on ARM Cortex-M4 microcontrollers, memory utilization analysis, and protocol-level overhead assessment.}
}\DIFdelend \DIFaddbegin \DIFadd{performance benchmarking on ARM Cortex-M4 microcontrollers, memory utilization analysis, and protocol-level overhead assessment.
}\DIFaddend
...
\DIFdelbegin \DIFdel{\bluet{Experimental results evaluate ML-DSA integration within MQTT-based IoT systems on ARM Cortex-M4 microcontrollers. Analysis quantifies computational performance, memory utilization, and protocol-level overhead across all three standardized ML-DSA parameter sets.}
}\DIFdelend \DIFaddbegin \DIFadd{Experimental results evaluate ML-DSA integration within MQTT-based IoT systems on ARM Cortex-M4 microcontrollers. Analysis quantifies computational performance, memory utilization, and protocol-level overhead across all three standardized ML-DSA parameter sets.
}\DIFaddend
...
Signature generation benefits from NTT optimization techniques, with each rejection sampling iteration requiring multiple forward and inverse NTT operations for polynomial arithmetic. \DIFdelbegin \DIFdel{\bluet{Optimized NTT implementations reduce per-iteration computational cost by 40--50\%, translating to proportional signing latency reduction.} However, \bluet{absolute signing latencies of 657--1,150\,ms across parameter sets remain 70--122$\times$ slower than ECDSA despite optimization},}\DIFdelend \DIFaddbegin \DIFadd{Optimized NTT implementations reduce per-iteration computational cost by 40--50\%, translating to proportional signing latency reduction. However, absolute signing latencies of 657--1,150\,ms across parameter sets remain 70--122$\times$ slower than ECDSA despite optimization, }\DIFaddend establishing performance constraints for signature-based IoT authentication. Performance variability from rejection sampling (expected 3.85--5.1 iterations depending on parameter set) introduces timing non-determinism requiring worst-case latency analysis for real-time applications.
...
Unlike signature generation, verification executes deterministically without rejection sampling, yielding predictable latency characteristics for real-time constraint analysis. Verification operations benefit from precomputed twiddle factors and optimized NTT implementations, \DIFdelbegin \DIFdel{\bluet{achieving 26--44$\times$ overhead relative to ECDSA compared to 71--122$\times$ overhead for signature generation}. \bluet{Absolute verification latencies of 416--717\,ms exceed sub-second bounds for interactive IoT applications across all parameter sets}}\DIFdelend \DIFaddbegin \DIFadd{achieving 26--44$\times$ overhead relative to ECDSA compared to 71--122$\times$ overhead for signature generation. Absolute verification latencies of 416--717\,ms exceed sub-second bounds for interactive IoT applications across all parameter sets}\DIFaddend , necessitating architectural accommodations for post-quantum authenticated messaging in latency-sensitive deployments.

...
\begin{table}[h]
	...
	\begin{tabular}{llcc}
		\hline
		\textbf{Level} & \textbf{Message Types}          & \textbf{Min. Security} & \textbf{Default} \\
		\hline
		Critical       & Firmware, credentials, keys     & ML-DSA-87              & ML-DSA-87        \\
		High           & Alerts, configuration, commands & ML-DSA-65              & ML-DSA-87        \\
		Medium         & Aggregated data, status reports & ML-DSA-44              & ML-DSA-65        \\
		Low            & Routine telemetry, heartbeats   & ML-DSA-44              & ML-DSA-44        \\
		\hline
	\end{tabular}
\end{table}
...
\subsection{Adaptive Selection Algorithm}

Algorithm~\ref{alg:adaptive-select} presents the adaptive security level selection procedure invoked prior to each signature generation operation.

\begin{algorithm}[h]
	\caption{Adaptive-ML-DSA-Select($msg$, $topic$, $state$)}
	\label{alg:adaptive-select}
	\begin{algorithmic}[1]
		\REQUIRE Message $msg$, MQTT topic $topic$, device state $state = (E, M, T)$
		\ENSURE Selected parameter set $param \in \{44, 65, 87\}$
		\STATE $C \leftarrow \text{GetCriticality}(topic)$ \COMMENT{Lookup criticality from topic}
		\STATE $(min\_level, default\_level) \leftarrow \text{SecurityPolicy}[C]$
		\STATE $R \leftarrow \alpha \cdot E + \beta \cdot M + \gamma \cdot (1 - T)$ \COMMENT{Resource score}
		\IF{$C = \text{CRITICAL}$}
		\RETURN $87$ \COMMENT{Critical messages always use ML-DSA-87}
		\ENDIF
		\IF{$R \geq 0.7$}
		\STATE $selected \leftarrow default\_level$ \COMMENT{Sufficient resources}
		\ELSIF{$R \geq 0.4$}
		\STATE $selected \leftarrow \max(min\_level, default\_level - 1)$ \COMMENT{Moderate resources}
		\ELSE
		\STATE $selected \leftarrow min\_level$ \COMMENT{Constrained resources}
		\ENDIF
		\IF{$M < 0.27$ \AND $selected > 44$}
		\STATE $selected \leftarrow 44$ \COMMENT{Memory constraint override}
		\ELSIF{$M < 0.53$ \AND $selected > 65$}
		\STATE $selected \leftarrow 65$ \COMMENT{Memory constraint override}
		\ENDIF
		\RETURN $selected$
	\end{algorithmic}
\end{algorithm}

The algorithm first retrieves message criticality from the MQTT topic hierarchy and looks up the corresponding minimum and default security levels. The resource score $R$ is computed from current device state. Critical messages bypass resource-based selection, always employing ML-DSA-87. For non-critical messages, resource score thresholds determine parameter selection: high resource availability ($R \geq 0.7$) permits default security levels, moderate availability ($0.4 \leq R < 0.7$) triggers one-level downgrade (respecting minimums), and low availability ($R < 0.4$) selects minimum security levels. Memory constraints impose hard overrides regardless of other factors, as insufficient memory prevents algorithm execution.

\subsection{Protocol Integration with MQTT}

The adaptive protocol integrates with MQTT publish operations through a wrapper function intercepting signature generation requests. Figure~\ref{fig:adaptive-flow} illustrates the integration architecture.
...
\begin{figure}[htbp]
	...
	\end{tikzpicture}
	\caption{Adaptive protocol integration with MQTT publish workflow. Message classification and resource assessment precede parameter selection, with selected security level passed to ML-DSA signing operation.}
	\label{fig:adaptive-flow}
\end{figure}

The protocol overhead comprises topic string parsing for criticality lookup (12--18 $\mu$s), resource state sampling from hardware registers (8--15 $\mu$s), and selection algorithm execution (3--5 $\mu$s), totaling 23--38 $\mu$s per message. This overhead represents less than 0.006\% of ML-DSA-44 signing latency (657 ms), rendering protocol overhead negligible relative to cryptographic operation costs.
...
\begin{table}[h]
	...
	\begin{tabular}{lccccc}
		\hline
		\textbf{Workload} & \textbf{Resource}        & \textbf{Fixed-87} & \textbf{Adaptive} & \textbf{Reduction} \\
		                  & \textbf{State}           & \textbf{(ms)}     & \textbf{(ms)}     &                    \\
		\hline
		Environmental     & High ($R \geq 0.7$)      & 1,136             & 732               & 35.6\%             \\
		Environmental     & Medium ($R \approx 0.5$) & 1,136             & 668               & 41.2\%             \\
		Environmental     & Low ($R < 0.4$)          & 1,136             & 678               & 40.3\%             \\
		\hline
		Industrial        & High                     & 1,136             & 849               & 25.3\%             \\
		Industrial        & Medium                   & 1,136             & 781               & 31.2\%             \\
		Industrial        & Low                      & 1,136             & 743               & 34.6\%             \\
		\hline
		Security          & High                     & 1,136             & 956               & 15.8\%             \\
		Security          & Medium                   & 1,136             & 891               & 21.6\%             \\
		Security          & Low                      & 1,136             & 847               & 25.4\%             \\
		\hline
	\end{tabular}
\end{table}
...
\begin{table}[h]
	...
	\begin{tabular}{lcccc}
		\hline
		\textbf{Workload} & \textbf{ML-DSA-44} & \textbf{ML-DSA-65} & \textbf{ML-DSA-87} & \textbf{Avg. Level} \\
		\hline
...
\begin{table}[h]
	...
	\begin{tabular}{lcccc}
		\hline
		\textbf{Workload} & \textbf{Fixed-87} & \textbf{Adaptive} & \textbf{Savings} & \textbf{Lifetime}  \\
		                  & \textbf{(J/day)}  & \textbf{(J/day)}  & \textbf{(J/day)} & \textbf{Extension} \\
		\hline
		Environmental     & 5.68              & 3.66              & 2.02             & +55.2\%            \\
		Industrial        & 5.68              & 4.25              & 1.43             & +33.6\%            \\
		Security          & 5.68              & 4.78              & 0.90             & +18.8\%            \\
		\hline
	\end{tabular}
\end{table}

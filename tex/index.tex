...
\begin{enumerate}
	\item \textbf{Adaptive Security Level Selection Protocol}: A protocol dynamically selecting ML-DSA parameter sets based on device resource state and message criticality, reducing computational overhead by 15.8--41.2\% compared to fixed highest-security configurations while maintaining minimum security guarantees.
...
The remainder of this paper is organized as follows: Section~\ref{sec:related} presents background and related work. Section~\ref{sec:architecture} describes the system architecture and adaptive protocol. Section~\ref{sec:evaluation} presents experimental evaluation. Section~\ref{sec:conclusion} concludes with implications for deployment.
...
\subsection{Adaptive Security Level Selection}\label{sec:adaptive}

Fixed security parameter selection imposes unnecessary overhead when uniform highest-security configurations are applied regardless of message characteristics. The adaptive protocol dynamically selects ML-DSA parameter sets based on message criticality and device resource availability.
...
\subsubsection{Resource State Assessment}
...
\begin{algorithm}[h]
	\caption{Adaptive Security Level Selection}
	\label{alg:adaptive}
	\SetKwInOut{Input}{Input}
	\SetKwInOut{Output}{Output}
	\Input{Message criticality $C$, Resource score $R$}
	\Output{ML-DSA parameter set $P$}
	$P_{\min} \leftarrow$ MinimumLevel($C$)\;
	$P_{\text{def}} \leftarrow$ DefaultLevel($C$)\;
	\uIf{$R \geq 0.7$}{
		$P \leftarrow P_{\text{def}}$\;
	}
	\uElseIf{$R \geq 0.4$}{
		$P \leftarrow \max(P_{\min}, P_{\text{def}} - 1)$\;
	}
	\Else{
		$P \leftarrow P_{\min}$\;
	}
...
\subsubsection{Adaptive Protocol Evaluation}

\paragraph{Workload Profiles}

Evaluation employs three representative IoT workload profiles: \textbf{Environmental Monitoring} (80\% low-criticality telemetry, 15\% medium, 4\% high, 1\% critical), \textbf{Industrial Control} (60\% low, 20\% medium, 15\% high, 5\% critical), and \textbf{Security-Sensitive} (40\% low, 30\% medium, 20\% high, 10\% critical).

\paragraph{Overhead Reduction}

Table~\ref{tab:adaptive-overhead} presents average signing latency under adaptive protocol compared to fixed ML-DSA-87 baseline.

\begin{table}[h]
	\centering
	\caption{Adaptive Protocol Overhead Reduction}
...
Table~\ref{tab:adaptive-energy} presents energy consumption for 100 daily messages.
...
\paragraph{Security Level Distribution}
...
\begin{table}[h]
	...
	\begin{tabular}{lcccc}
		\toprule
		\textbf{Workload} & \textbf{DSA-44} & \textbf{DSA-65} & \textbf{DSA-87} & \textbf{Avg. Level} \\
		\midrule

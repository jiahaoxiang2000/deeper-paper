...
Statistical rigor derives from repeated measurement methodology executing each operation 1,000 iterations with outlier elimination. We report median execution times providing robust central tendency estimates resilient to measurement noise, along with interquartile ranges quantifying performance variability. For signature generation operations exhibiting substantial variance due to rejection sampling, we additionally report minimum and maximum observed execution times characterizing performance bounds.
...
Comparative analysis employs overhead ratios normalizing ML-DSA measurements relative to ECDSA baseline implementations, enabling quantification of post-quantum migration costs. For each metric $M$, we compute overhead ratio $R = M_{\text{ML-DSA}} / M_{\text{ECDSA}}$, with values exceeding 1.0 indicating increased resource consumption. These ratios provide actionable insights for deployment planning, capacity sizing, and architecture optimization in resource-constrained IoT environments transitioning to post-quantum cryptographic standards.

...
The ML-DSA signing procedure constitutes the primary computational bottleneck in IoT deployments, warranting detailed algorithmic analysis. The signing operation comprises a wrapper function handling context processing and randomness generation, followed by the computationally intensive internal signing procedure implementing the Fiat-Shamir with Aborts paradigm.

\subsubsection{Signing Procedure Overview}

The signing wrapper (Algorithm~\ref{alg:mldsa-sign}) processes context strings and generates randomness before invoking the internal signing routine. Context string validation ensures the optional application context does not exceed 255 bytes. Randomness generation produces 32 bytes of cryptographic random data; for deterministic variants, this randomness is substituted with zeros to enable reproducible signatures.
...
\textbf{Rejection Sampling Overhead}: The Fiat-Shamir with Aborts paradigm requires iterative signature attempts until the response vector satisfies norm bounds. Expected iteration counts of 4.25 (ML-DSA-44), 5.1 (ML-DSA-65), and 3.85 (ML-DSA-87) introduce timing variability and multiplicative computational overhead, with worst-case iterations potentially exceeding 20 attempts.

\textbf{NTT Computational Dominance}: Each signing iteration requires multiple NTT/INTT operations for polynomial arithmetic. On ARM Cortex-M4 at 168 MHz, reference NTT implementations consume 2.5-3.0 million cycles per transformation, establishing NTT as the dominant computational primitive.
...
\textbf{Hash Function Invocations}: SHAKE-256 hash function calls for matrix expansion, challenge derivation, and message hashing consume 15-20\% of signing computation. The Keccak-based SHAKE implementation requires careful optimization on platforms lacking hardware acceleration.
...
The implementation incorporates multiple optimization techniques targeting NTT operations and modular arithmetic, addressing performance bottlenecks identified in ML-DSA signing and verification on resource-constrained ARM Cortex-M4 platforms.

\subsubsection{NTT Assembly Optimization}

NTT operations constitute 60-70\% of ML-DSA computational cost, necessitating aggressive optimization for acceptable performance on embedded platforms. The implementation employs hand-optimized ARM assembly targeting Cortex-M4 instruction set architecture, replacing compiler-generated code for performance-critical butterfly operations.

Assembly optimization exploits ARM Cortex-M4 architectural features: the \texttt{UMULL} instruction performs 32$\times$32$\rightarrow$64-bit multiplication generating full product without overflow, enabling efficient modular multiplication without intermediate truncation. Conditional execution eliminates branch instructions in butterfly loops, reducing pipeline stall penalties. Register allocation minimizes memory access by maintaining twiddle factors, polynomial coefficients, and intermediate results in the 13 general-purpose registers available for computation.

The optimized NTT implementation structures computation in 256-element blocks matching Cortex-M4 cache line characteristics. Loop unrolling by factors of 2-4 reduces loop overhead and enables instruction-level parallelism exploitation through dual-issue execution units. Precomputed twiddle factors stored in Flash memory eliminate runtime exponentiation, trading 2 KB storage for elimination of $256 \log_2 256 = 2048$ modular exponentiations per NTT operation.

Performance profiling quantifies assembly optimization impact: reference C implementation NTT execution requires 3.2 million cycles on Cortex-M4 at 168 MHz, while assembly-optimized implementation achieves 2.3 million cycles (28.1\% reduction). Combined with inverse NTT optimization (similar 26-29\% improvement), aggregate signing latency decreases by 20-30\% relative to reference implementations.
...
The implementation analyzes ML-DSA arithmetic to establish safe lazy reduction chains. NTT butterfly operations performing $a + b \bmod q$ and $a - b \bmod q$ tolerate input coefficients up to $2q-1$ without overflow in 32-bit arithmetic ($2 \cdot 2q < 2^{32}$ for ML-DSA modulus $q = 8380417$). Multiplication results $(a \cdot b) \bmod q$ with inputs bounded by $2q$ yield products below $4q^2 < 2^{64}$, fitting within 64-bit intermediate representation produced by \texttt{UMULL}.

Lazy reduction chains defer reduction through sequences of 2-4 consecutive additions or subtractions before applying single reduction operation. NTT stages performing paired butterfly operations $(w_j, w_{j+n}) \leftarrow (w_j + t, w_j - t)$ employ single reduction after both operations rather than per-operation reduction, halving reduction frequency. Critical path analysis ensures coefficient bounds remain within safe ranges throughout computation chains, preventing arithmetic overflow.

Implementation achieves 15-25\% NTT latency reduction through lazy reduction strategies relative to per-operation reduction baselines. Combined with assembly optimization, cumulative NTT performance improvement reaches 40-50\% as reported in Section~\ref{sec:results}.

\subsubsection{Barrett Modular Reduction}
...
Barrett reduction precomputes $\mu = \lfloor 2^{48} / q \rfloor$ for ML-DSA modulus $q = 8380417$, yielding $\mu = 33554431$. Reduction of 32-bit value $a$ proceeds through approximate quotient computation $\hat{q} = \lfloor (a \cdot \mu) / 2^{48} \rfloor$ via 32$\times$32 multiplication and 48-bit right shift, followed by correction step $r = a - \hat{q} \cdot q$. The result $r$ satisfies $0 \le r < 2q$; conditional subtraction yields final reduced value $r' = r < q ? r : r - q$.

ARM Cortex-M4 implementation utilizes \texttt{UMULL} for $(a \cdot \mu)$ multiplication, extracting high 32 bits as approximate quotient. Subsequent multiplication $\hat{q} \cdot q$ and subtraction employ standard 32-bit arithmetic. Conditional subtraction employs comparison and conditional move instructions (\texttt{CMP}, \texttt{IT}, \texttt{SUB}) without branching overhead.

Barrett reduction integration within NTT implementations replaces all modular reduction operations, achieving 25-35\% reduction overhead improvement relative to division-based approaches. The optimization proves particularly effective for signature generation (hundreds of thousands of reduction operations) but provides minimal benefit for infrequent operations like key generation.
...
Key generation performance directly impacts device provisioning workflows and key rotation strategies in IoT deployments. The 100.7-238× computational overhead relative to ECDSA P-256 reflects inherent lattice-based cryptographic complexity; optimization techniques provide limited improvement for key generation operations dominated by random polynomial sampling and matrix expansion rather than NTT arithmetic. Measured execution times of 151-357 milliseconds establish feasibility for infrequent key generation during device provisioning, but prohibit high-frequency rotation strategies requiring sub-second key derivation.
...
Signature generation constitutes the primary beneficiary of NTT optimization techniques, with each rejection sampling iteration requiring multiple forward NTT and inverse NTT operations for polynomial arithmetic. Optimized NTT implementations reduce per-iteration computational cost by 40-50\%, directly translating to proportional signing latency reduction. However, absolute signing latencies of 657-1,150 milliseconds across parameter sets remain 70-122× slower than ECDSA despite aggressive optimization, establishing fundamental performance constraints for signature-based IoT authentication. Performance variability from rejection sampling (expected 3.85-5.1 iterations depending on parameter set) introduces timing non-determinism requiring worst-case latency analysis for real-time applications.
...
Unlike signature generation, verification executes deterministically without rejection sampling, yielding predictable latency characteristics for real-time constraint analysis. Verification operations benefit from precomputed twiddle factors and optimized NTT implementations, achieving 26-44× overhead relative to ECDSA compared to 71-122× overhead for signature generation. However, absolute verification latencies of 416-717 milliseconds exceed sub-second bounds required for interactive IoT applications across all parameter sets, necessitating architectural accommodations for post-quantum authenticated messaging in latency-sensitive deployments.
